...
def extract_hand_features(landmarks):
    ...
    palm_width = distance(landmarks[5], landmarks[17]) or 1e-6
    return np.array(
        [
...
def precompute_features(dataset_split, split_name="train"):
    print(
        f"🟡 Precomputing features for {len(dataset_split)} samples ({split_name})...",
        flush=True,
    )
    features = []
    start = time.time()
    ...
    for i in range(len(dataset_split)):
        ...
        try:
            ...
            if not results.multi_hand_landmarks:
                ...
                continue
            features.append(
                extract_hand_features(results.multi_hand_landmarks[0].landmark)
            )
        except:
            features.append(np.zeros(10, dtype=np.float32))

        if i % 200 == 0 or i == len(dataset_split) - 1:
            elapsed = time.time() - start
            print(
                f"   ⏱️ [{split_name}] {i+1}/{len(dataset_split)} processed — {elapsed:.1f}s elapsed",
                flush=True,
            )

    print(
        f"✅ Done with {split_name}: {len(features)} features in {time.time() - start:.1f}s\n",
        flush=True,
    )
